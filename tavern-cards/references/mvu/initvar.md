...
- 需要与用户逐组确认初值时，走 `references/mvu/initvar-design.md` 的设计流程（含初值确认对话与 YAML 格式化纪律）

### 初值纪律（墨月增补）

- 不以「未知」代替 boolean——真假字段必须确认成立或不成立
- 不把未决状态统一改成空字符串、null 或「待初始化」——按实际 Schema 语义处理
- 区分结构用于缺失输入的默认值（`.prefault(...)`）与剧情真正的开局值——只有适合当前开局时才采用
- 数字用 YAML 数字、真假用 `true`/`false`；日期、时间、前导零和易被误读为其他类型的文本加引号
- 动态映射开局为空用 `{}`，有序列表开局为空用 `[]`；枚举值使用 Schema 中的原文字面值
