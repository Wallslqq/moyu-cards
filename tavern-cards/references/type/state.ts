...
export const TavernCardsState = z.object({
  ...
  personality: z.string().optional().describe('性格摘要'),
  scenario: z.string().optional().describe('情景设定'),
  first_messages: z.array(z.string()).default([]).describe('开场白内容/文件路径数组, [0]=first_mes, [1:]=alternate_greetings'),
  mes_example: z.string().optional().describe('对话示例'),
  system_prompt: z.string().optional().describe('角色卡系统提示, pack 时仅写入 data.*'),
  post_history_instructions: z.string().optional().describe('历史后指令 / PHI, pack 时仅写入 data.*'),
  tags: z.array(z.string()).optional().describe('角色标签'),
