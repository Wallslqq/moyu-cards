...
墨月写卡知识来源于「墨月写卡器 · 送君一程，有缘自会再相见」（经作者授权移植，见下方说明）。

## 与上游的差异

本仓库是 [ai4rpg/tavern-cards](https://github.com/ai4rpg/tavern-cards) 的完整 fork，将墨月写卡器的 35 项创作知识移植进 skill 架构（**经墨月作者授权**）。上游全部能力与流程保留不动，新增：

- **人物生境路线**：反模板、反预写答案的人物条目方法论（基础信息 / 生活结构 / 人物性情 / 场景表达 / 穿衣风格五专项 + NPC 轻量版），与原调色盘路线并存，项目级二选一（创作规划 `project.character_route`）
- **世界观规模分级**：大 / 中 / 小三档方法论与规模路由，从体验反推必要世界
- **MVU 对话式设计层**：schema 结构大白话逐轮确认、初值与更新规则设计终稿、Zod 写法增补；新增 `mvu-check-agent` 三文件交叉检查子代理
- **单 HTML 状态栏（`ui_mode: simple`）**：零工程门槛的原生 / Vue 双线状态栏，与 text / frontend 并列
- **消息前端**：信件、日志、论坛等消息内渲染的单 HTML 前端范式
- **开场白文风确认**：先定文风再写正文，附使用者边界
- **工坊运行总纲**：每轮一问、不重复确认、条件齐备主动停止等对话纪律

墨月知识的提取、改写与移植验证过程档案见 `移植留档/`。
...
- 上游 [ai4rpg/tavern-cards](https://github.com/ai4rpg/tavern-cards) 的全部内容遵循其「仅供个人使用，二次修改需注明出处」约束
- 墨月创作知识（人物生境、世界观规模、MVU 设计对话、单 HTML 状态栏、消息前端等）移植自「墨月写卡器 · 送君一程，有缘自会再相见」，**经墨月作者授权**随本仓库分发；墨月的程序代码部分（GPL-3.0）未被包含于本仓库