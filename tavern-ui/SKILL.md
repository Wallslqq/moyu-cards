...
前端开发基于 `tavern_helper_template` 共享基础设施仓库。该仓库同时服务于多个角色卡项目，`src/` 之外均为共享配置。明确权限范围，避免修改一个项目时波及其它项目。

**遇到问题时的处理流程**：

1. 先在 `src/{ProjectName}/` 内寻找解法（大多数问题可在项目内解决）
2. 若项目内无法解决，先查看 tavern_helper_template 在 GitHub 上的更新说明，确认是否有能解决该问题的更新；有则通过拉取更新解决
3. 如确认必须修改共享文件，先告知用户影响范围与原因，经确认后再执行
